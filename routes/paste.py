...
from models import Paste, User, db
...
def index():
    ...
    if request.method == "POST":
        ...
        return jsonify(
            {
                "success": True,
                "redirect_url": url_for("paste.view_paste", paste_id=paste_id),
            }
        )
...
def view_paste(paste_id):
    ...
    if "_user_id" in session:
        current_user = User.query.get(session["_user_id"])
        if current_user:
...
def transfer_ownership(paste_id):
    ...
    current_user_obj = User.query.get(session.get("_user_id"))
